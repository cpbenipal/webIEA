...
class EventManagerContactsAdvancedSearch extends EventManagerBase {
    constructor(args) {
        super();
    ...
    linkFolder(args) {
        ...
        args.callbackSucceed = function (data, status, xhr, _args, response) {
            if (response.result == true) {
                ...
                fp_ConfirmDialog('Covert',
                    'The selected folder is not distribution list. Do you want to convert it to distribution list?',
                    function () {
    ...
    static removeDiagramHistory(args) {
        args.nameHistory = $("#HistoryName").val();
        args.historyStart = $("#HistoryStart").val();
        args.userName = $("#UserName").val();
        var step = 0;
    ...
    static getHistoryBPMNxmls(args) {
        args.BPMNxmls = [];
        var step = 0;
        args.temlBPMNXml = null;
        do {
    ...
    static setHistoryStepArrows(args) {
