...
function fp_initSetting(setting, value) {
    if (setting) fp_settings[setting] = value;
}

function isPageEmpty() {
    let blockList  = document.getElementById('BlockList'+currentModelID);
    let childCount = blockList.childElementCount;

    for (let i = 0; i < childCount; i++) {
        let currentChild = blockList.children[i];
        if (currentChild.classList.contains('flexpage') && (currentChild.childElementCount > 1))
        {
            return false;
        }
    }
...
function fp_ConfirmDialog(title, text, onconfirm, oncancel) {
    ...
    var dialog = $('<div class="modal fade" id="fp_confirm_dialog" tabindex="-1" role="dialog" ></div>').append($('<div class="modal-dialog mini"></div>').append($('<div class="modal-content"></div>').append($('<div class="modal-header"></div>').append($('<h3 class="modal-title"></h3>').html(title))).append($('<div class="modal-body"></div>').append($('<p>').html(text))).append($('<div class="modal-footer"></div>').append($('<div class="col-md-6 pull-right right"></div>').append($('<a href="javascript:void(0)" data-dismiss="modal" class="ok_btn">OK</a>')).append($('<a href="javascript:void(0)" data-dismiss="modal" aria-label="Close">CANCEL</a>')))))).appendTo($('body'));
...
function fp_TwoButtonsDialog(title, text, firstbutton, secondbutton, onconfirm) {
    ...
    var dialog = $('<div class="modal fade" id="fp_twoButtons_dialog" tabindex="-1" role="dialog" ></div>').append($('<div class="modal-dialog mini"></div>').append($('<div class="modal-content"></div>').append($('<div class="modal-header"></div>').append($('<h3 class="modal-title"></h3>').html(title))).append($('<div class="modal-body"></div>').append($('<p>').html(text))).append($('<div class="modal-footer"></div>').append($('<div class="col-md-6 pull-right right"></div>').append($('<a href="javascript:void(0)" data-dismiss="modal" class="first_btn">' + firstbutton + '</a > ')).append($('<a href="javascript:void(0)" data-dismiss="modal" class="second_btn">' + secondbutton + '</a > ')).append($('<a href="javascript:void(0)" data-dismiss="modal" aria-label="Close">CANCEL</a>')))))).appendTo($('body'));

    $('body > #fp_twoButtons_dialog .first_btn').on('click', function () {
        onconfirm(0);
    });
    $('body > #fp_twoButtons_dialog .second_btn').on('click', function () {
        onconfirm(1);
    });
...
var fp_loadScripts = function fp_loadScripts(url, type, afterFnc) {
    if (fp_settings.debug === true && url.substring(url.length - 7, url.length - 2) === ".min.") {
        url = url.substring(0, url.length - 6) + "js";
    }
    var head = document.getElementsByTagName("head")[0];
    var scripts = [].concat(_toConsumableArray(head.getElementsByTagName("script")));
    scripts = scripts.filter(function (scr) {
        return scr.getAttribute("src") === url;
    });
    if (scripts.length === 0) {
    ...
    if (type && afterFnc) {
        ...
        if (typeof typeValue !== 'undefined') {
            ...
            setTimeout(function () {
                fp_loadScripts(url, type, afterFnc);
            }, 1500);
        }
    }
};
