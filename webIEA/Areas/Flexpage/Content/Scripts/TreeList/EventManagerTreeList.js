...
class EventManagerTreeList extends EventManagerBase {
    constructor(args) {
        super();
    ...
    clickContextMenu(args) {
        ...
        if (args.event.item.name === "Create") {
            ...
            $(window.document).on("keydown", function (e) {
                ...
                if (e.keyCode === 13)
                    eval('fp_FolderTreeList_List' + args.ID).UpdateEdit();
            });
            /*EventManager.tools.showLoading(args);
            args.callbackSucceed = function (data, status, xhr, _args) {
            ...
            var nameFolder = $(".fp_textTreeListNode[data-rowid=" + args.rowId + "] span").text();
            fp_popupControlOpen({ command: 'edit', blocklistID: '0', blockType: 'ObjectProperties', blockAlias: '', blockID: args.rowId, title: 'Properties: ' + nameFolder },
                function (save) {
                    if (save) {
                        ...
                        $(".fp_browser .fp_contactsEnumeration").each((index, elem) => {
                            var _args = {};
                        ...
                        $(".fp_browser .fp_folderContent").each((index, elem) => {
                            var _args = {};
            ...
            args.callbackSucceed = function (data, status, xhr, _args) {
                ...
                fp_ConfirmDialog("Reset time to leave", "Do you want to make a recursive reset for all child objects?", function () {
                    args.requestParameters = {
    ...
    /**
      ...
      * @param {object} args UI options
      */
    renameApplyFolder(args) {
        if (args.event.type === "keydown" && (args.event.which == 13 || args.event.keyCode == 13)
            || args.event.type === "click" && $(args.event.target).hasClass("fp_textTreeListNodeInput") === false
     ...
     * @return {args} args UI options
     */
    static getExtendedfolderID(args) {
